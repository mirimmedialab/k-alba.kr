...
// ────────── 사장님 연락처 추가 안내 메일 (트랜잭셔널) ──────────
export function buildContactReminderEmail(who, siteUrl) {
  const manageUrl = `${siteUrl}/my/jobs`;
  const subject = `[K-ALBA] 공고에 연락처를 추가해 주세요`;
  const eyebrow = "연락처 확인 필요";
  const headline = `${esc(who)}님, 공고에<br>연락처를 추가해 주세요 📞`;
  const desc =
    '등록하신 공고에 <strong style="color:#0A1628;">지원자가 연락할 수단</strong>이 아직 없어요. 내 공고에서 전화·휴대전화·이메일 중 하나를 추가하면, 알바생이 바로 연락드릴 수 있어요.';
  const cta = "내 공고 관리하기 →";
  const note = "전화·휴대전화·이메일 중 하나만 있으면 돼요.";
  const manageLink = `<a href="${manageUrl}" target="_blank" style="color:#FF6B5A;font-weight:700;text-decoration:none;">내 공고로 이동 →</a>`;
  const finePrint = "본 메일은 회원님의 공고 등록에 따른 서비스 안내 메일입니다.";
  const rows =
    introRow(esc(eyebrow), headline, desc) +
    ctaRow(manageUrl, esc(cta), "#FF6B5A") +
    noteRow(esc(note), manageLink) +
    footerRow(finePrint);
  return { subject, html: emailShell(rows) };
}

// 연락수단 없이 direct/chatbot 공고를 올린 사장님들에게 안내 메일 발송.
export async function sendContactReminders(supabase, opts = {}) {
  ...
  const employerRedirect = (
    opts.employerRedirectTo ||
    process.env.EMAIL_REDIRECT_EMPLOYER_TO ||
  ...
  if (empIds.length === 0) return { ok: true, employers: 0, sent: 0 };
  ...
  const seen = new Set();
  for (const e of emps || []) {
    const to = employerRedirect || e.email;
    if (!to || !to.includes("@") || seen.has(to)) continue;
    seen.add(to);
    const who = e.company_name || e.name || "사장님";
    const { subject, html } = buildContactReminderEmail(who, siteUrl);
    const r = await sendViaResend({ from: FROM, to, reply_to: REPLY_TO, subject, html }, resendKey);
    if (r.ok) sent++;
    await new Promise((res) => setTimeout(res, 100));
  }
  return { ok: true, employers: empIds.length, sent };
}
