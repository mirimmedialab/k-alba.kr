...
const EXPRS = {
  bakery: [
    { ko: "갓 구운 빵입니다", en: "It's freshly baked bread", when: "방금 나온 빵을 안내할 때" },
    { ko: "포장해 드릴까요?", en: "Would you like it to go?", when: "포장 여부를 물어볼 때" },
    { ko: "케이크에 초 몇 개 드릴까요?", en: "How many candles would you like with the cake?", when: "케이크 구매 손님에게 초를 챙겨줄 때" },
    { ko: "유통기한은 오늘까지입니다", en: "The expiration date is today", when: "당일 판매 제품을 안내할 때" },
    { ko: "견과류가 들어 있어요", en: "It contains nuts", when: "알레르기 주의 성분을 안내할 때" },
    { ko: "시식해 보세요", en: "Please try a sample", when: "시식을 권할 때" },
    { ko: "봉투에 따로 담아드릴까요?", en: "Shall I put them in separate bags?", when: "여러 상품을 나눠 포장할지 물어볼 때" },
    { ko: "적립카드 있으세요?", en: "Do you have a point card?", when: "포인트 적립 여부를 물어볼 때" },
    { ko: "감사합니다. 또 오세요", en: "Thank you. Please come again", when: "손님을 배웅할 때" },
    { ko: "잠시만 기다려 주세요", en: "Please wait a moment", when: "새 빵이 나오기를 기다릴 때" },
  ],
  fastfood: [
    { ko: "주문 도와드릴까요?", en: "May I take your order?", when: "카운터에서 주문을 받을 때" },
    { ko: "세트로 드릴까요, 단품으로 드릴까요?", en: "Would you like a combo or just the item?", when: "세트 여부를 확인할 때" },
    { ko: "매장에서 드시고 가세요?", en: "For here?", when: "매장 이용 여부를 확인할 때" },
    { ko: "음료는 무엇으로 하시겠어요?", en: "What drink would you like?", when: "세트 음료를 고를 때" },
    { ko: "소스 추가하시겠어요?", en: "Would you like extra sauce?", when: "소스 추가 여부를 물어볼 때" },
    { ko: "번호 불러드리면 픽업대로 와주세요", en: "Please come to the pickup counter when your number is called", when: "번호표를 건네줄 때" },
    { ko: "잠시만 기다려 주세요. 새로 만들어 드릴게요", en: "One moment please, we'll make it fresh", when: "조리에 시간이 걸릴 때" },
    { ko: "죄송합니다. 주문을 다시 확인하겠습니다", en: "I'm sorry, let me check your order again", when: "주문이 잘못 나갔을 때" },
    { ko: "케첩은 셀프바에 있습니다", en: "Ketchup is at the self-service bar", when: "셀프바 위치를 안내할 때" },
    { ko: "감사합니다. 맛있게 드세요", en: "Thank you, enjoy your meal", when: "주문을 건네주며 인사할 때" },
  ],
  convenience: [
...
const READINGS = {
  bakery: [
    { passage: "[당일 폐기 안내]\n크림빵: 오늘 21:00 폐기\n식빵: 내일 12:00 폐기\n케이크: 냉장 보관 3일", q: "오늘 밤 9시에 폐기해야 하는 것은?", correct: "크림빵", wrong: ["식빵", "케이크", "전부"] },
    { passage: "[알레르기 표시]\n아몬드 크루아상: 견과류 포함\n플레인 베이글: 견과류 없음", q: "견과류 알레르기 손님에게 권할 수 있는 것은?", correct: "플레인 베이글", wrong: ["아몬드 크루아상", "둘 다 가능", "둘 다 불가"] },
    { passage: "[진열 규칙]\n1. 새 빵은 뒤쪽에 진열\n2. 기존 빵을 앞으로 당기기\n3. 빈 트레이는 바로 회수", q: "새로 나온 빵은 어디에 진열하나요?", correct: "뒤쪽", wrong: ["앞쪽", "아무 곳", "창가"] },
    { passage: "[예약 주문]\n7/26 14:00 생일 케이크 1호 (초 10개) 김OO님", q: "예약 케이크와 함께 준비할 것은?", correct: "초 10개", wrong: ["초 5개", "폭죽", "풍선"] },
    { passage: "[마감 체크]\n1. 진열대 정리  2. 남은 빵 기록  3. 매장 청소  4. 포스 정산", q: "마감 체크리스트에 없는 일은?", correct: "신제품 굽기", wrong: ["진열대 정리", "포스 정산", "매장 청소"] },
  ],
  fastfood: [
    { passage: "[세트 구성]\n버거세트 = 버거 + 감자튀김(M) + 음료(M)\n음료 변경: 콜라/사이다/제로콜라", q: "버거세트에 포함되지 않는 것은?", correct: "치킨너겟", wrong: ["감자튀김(M)", "음료(M)", "버거"] },
    { passage: "[튀김 조리 시간]\n감자튀김: 3분\n치킨너겟: 4분\n어니언링: 2분 30초", q: "조리 시간이 가장 짧은 것은?", correct: "어니언링", wrong: ["감자튀김", "치킨너겟", "모두 같음"] },
    { passage: "[위생 규칙]\n튀김유는 매일 오전 교체. 바닥에 기름이 떨어지면 즉시 닦고 미끄럼 주의 표지판을 세우세요.", q: "바닥에 기름이 떨어졌을 때 해야 할 일은?", correct: "즉시 닦고 주의 표지판을 세운다", wrong: ["퇴근 후 닦는다", "그대로 둔다", "표지판만 세운다"] },
    { passage: "[주문 화면]\n#37 불고기버거 세트 1 (콜라 → 제로콜라 변경), 소스 추가 1", q: "주문 내용으로 알맞은 것은?", correct: "불고기버거 세트, 제로콜라, 소스 추가", wrong: ["새우버거 세트", "콜라 그대로", "소스 없음"] },
    { passage: "[교대 수칙]\n교대 15분 전 도착 → 손 세척 → 유니폼 착용 → 인수인계", q: "교대 시 가장 먼저 해야 할 일은?", correct: "15분 전 도착", wrong: ["인수인계", "유니폼 착용", "손 세척"] },
  ],
  convenience: [
...
const LISTENINGS = {
  bakery: [
    { tts: "소보로빵 두 개랑 우유식빵 하나 주세요.", q: "손님이 주문한 것은?", correct: "소보로빵 2개, 우유식빵 1개", wrong: ["소보로빵 1개, 우유식빵 2개", "크림빵 2개", "케이크 1개"] },
    { tts: "이 케이크 오늘 만든 건가요?", q: "손님이 알고 싶은 것은?", correct: "케이크를 만든 날짜", wrong: ["케이크 가격", "케이크 크기", "포장 여부"] },
    { tts: "생일 케이크인데 초 일곱 개 넣어주세요.", q: "함께 챙겨야 할 것은?", correct: "초 7개", wrong: ["초 17개", "포크 7개", "접시 7개"] },
    { tts: "견과류 알레르기가 있는데 이 빵 먹어도 되나요?", q: "알바생이 확인해야 할 것은?", correct: "빵에 견과류가 들어 있는지", wrong: ["빵의 가격", "빵의 크기", "유통기한만"] },
    { tts: "식빵은 반으로 잘라서 따로 포장해 주세요.", q: "손님의 요청으로 알맞은 것은?", correct: "식빵을 반으로 잘라 따로 포장", wrong: ["식빵을 통째로 포장", "식빵 교환", "식빵 환불"] },
  ],
  fastfood: [
    { tts: "불고기버거 세트 하나, 음료는 제로콜라로 바꿔주세요.", q: "주문 내용으로 알맞은 것은?", correct: "불고기버거 세트, 음료는 제로콜라", wrong: ["새우버거 단품", "콜라 그대로", "감자튀김만"] },
    { tts: "감자튀김에 소금 빼주세요.", q: "손님의 요청은?", correct: "감자튀김에 소금을 넣지 않기", wrong: ["소금 추가", "감자튀김 큰 사이즈", "케첩 추가"] },
    { tts: "주문하신 37번 손님, 픽업대로 와주세요.", q: "이 안내의 의미는?", correct: "37번 주문이 준비되어 찾아가면 된다", wrong: ["37번 주문이 취소되었다", "37번 손님이 나가야 한다", "37번 자리로 앉으라는 뜻"] },
    { tts: "햄버거에서 피클 빼고 양파 추가해 주세요.", q: "주문 내용으로 알맞은 것은?", correct: "피클 제외, 양파 추가", wrong: ["피클 추가, 양파 제외", "둘 다 추가", "둘 다 제외"] },
    { tts: "죄송하지만 아이스크림 기계가 지금 고장이에요.", q: "안내한 내용으로 알맞은 것은?", correct: "아이스크림을 지금 판매할 수 없다", wrong: ["아이스크림이 할인 중이다", "아이스크림이 새로 나왔다", "아이스크림이 무료다"] },
  ],
  convenience: [
