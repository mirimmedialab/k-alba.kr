...
import { getContract, updateContract, signContract, getCurrentUser } from "@/lib/supabase";
...
export default function ContractDetailPage() {
  ...
  const [tab, setTab] = useState("chatbot"); // chatbot | form | preview
  ...
  const handleSign = async (role) => {
    ...
    addUser(role === "worker" ? "✍️ 서명하기" : "✍️ 사장님 서명");

    const { error } = await signContract(contract.id, role);
    ...
    setTimeout(() => {
      if (role === "worker") {
        ...
        setTimeout(() => addBot("👨‍💼 사장님이 서명하시면 계약이 완료됩니다."), 1000);
      } else {
        addBot("🎉 계약 완료!\n📄 PDF 양측 발송됨\n✅ K-ALBA 근무 기록 자동 저장!");
      }
    }, 500);
