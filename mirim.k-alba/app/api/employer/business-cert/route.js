...
export async function POST(req) {
  try {
    ...
    const storagePath = `${user.id}/business_cert_${Date.now()}.${ext}`;
    ...
    const { data: prev } = await supabase
      .from("profiles").select("business_cert_path").eq("id", user.id).maybeSingle();
    if (prev?.business_cert_path && prev.business_cert_path !== storagePath) {
      await supabase.storage.from(BUCKET).remove([prev.business_cert_path]).catch(() => {});
    }

    const { error: dbErr } = await supabase
      .from("profiles")
      .update({
        business_cert_path: storagePath,
        business_cert_name: file.name,
        business_cert_uploaded_at: new Date().toISOString(),
      })
      .eq("id", user.id);
    if (dbErr) {
  ...
  try {
    ...
    if (applicationId) {
      ...
      const { data: employers } = await supabase
        .from("profiles")
        .select("id, business_number, business_cert_path, business_cert_name, business_cert_uploaded_at")
      ...
      const { data: me } = await supabase
        .from("profiles")
        .select("id, business_cert_path, business_cert_name, business_cert_uploaded_at")
        .eq("id", user.id)
        .maybeSingle();
      employerProfile = me;
    }

    if (!employerProfile?.business_cert_path) {
      return NextResponse.json({ ok: true, exists: false });
    }

    const { data: urlData, error: urlErr } = await supabase.storage
      .from(BUCKET)
      .createSignedUrl(employerProfile.business_cert_path, 60 * 60);
    if (urlErr) {
      return NextResponse.json({ ok: false, error: "링크 생성 실패: " + urlErr.message }, { status: 500 });
    }

    return NextResponse.json({
      ok: true,
      exists: true,
      file_name: employerProfile.business_cert_name,
      uploaded_at: employerProfile.business_cert_uploaded_at,
